      ...
      <div className={`grid grid-cols-1 ${activeEvent.mediaLink ? 'lg:grid-cols-3' : 'lg:grid-cols-2'}`}>
        <div className="relative h-64 lg:h-auto overflow-hidden lg:col-span-1">
          <Image
            ...
            fill
            className={`object-cover transition-transform duration-700 ${!isPast ? 'group-hover:scale-105' : ''} ${isPast ? 'grayscale-[50%]' : ''}`}
            loading="lazy"
          />
...
export default function EventsContent({ initialEvents }: { initialEvents: Event[] }) {
  ...
  return (
    <div className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-[80vh] flex flex-col pt-32">
      ...
      {isLoading && initialEvents.length === 0 ? (
            ...
            >
              {(() => {
                ...
                return isPastEvent ? (
                  ...
                  <Link href={`/events/${activeEvent.id}`} className="block group cursor-pointer">
                    <CardContent
                      activeEvent={activeEvent}
                      activeTab={activeTab}
                      likedEvents={likedEvents}
