...
export const SummerBuddyUpDashboard: React.FC<SummerBuddyUpDashboardProps> = ({ team, currentUser, onRefreshTeam }) => {
  ...
  const handleCheckInSubmit = async (e: React.FormEvent, milestone: 'JULY_15' | 'JULY_30' | 'AUGUST_15' | 'AUGUST_30') => {
    ...
    if (!ytRegex.test(youtubeUrl)) {
      setCheckInError(tr('Please enter a valid YouTube video URL.', 'Por favor ingresa un enlace válido de video de YouTube.'));
      return;
    }
    ...
    for (const q of buddyUpConfig.checkin_questions) {
      if (!answers[q] || answers[q].trim().length < 5) {
        setCheckInError(tr(`Please provide a more detailed answer for: "${q}"`, `Por favor da una respuesta más detallada para: "${q}"`));
        return;
      }
    }
    ...
    try {
      ...
      console.error(err);
      setCheckInError(err.message || tr('Failed to submit check-in.', 'No se pudo enviar el reporte de avance.'));
    } finally {
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onRefreshTeam();
      loadData();
      alert(tr(`Successfully added new ${addMemberType}! The team is now pending Admin Approval.`, `¡Se agregó correctamente ${addMemberType === 'student' ? 'el nuevo estudiante' : 'el nuevo sub-entrenador'}! El equipo ahora está pendiente de Aprobación del Administrador.`));
      ...
      console.error(err);
      alert(tr('Failed to add member. Please try again.', 'No se pudo agregar al miembro. Por favor intenta de nuevo.'));
    } finally {
  ...
  const copyInviteText = () => {
    ...
    const text = tr(
      `Hey co-coach! I registered our Aaria's Blue Elephant Summer Buddy Up team ("${team.team_name}"). Please log in to ${inviteLink} using your Google account (${currentUser.id === team.head_coach_id ? 'the email I added' : 'your email'}) to claim your profile and accept the safety waivers so we can start tracking our check-ins! 🐘💙`,
      `¡Hola, compañero entrenador! Registré nuestro equipo de Summer Buddy Up de Aaria's Blue Elephant ("${team.team_name}"). Por favor inicia sesión en ${inviteLink} con tu cuenta de Google (${currentUser.id === team.head_coach_id ? 'el correo que agregué' : 'tu correo'}) para reclamar tu perfil y aceptar las exenciones de seguridad para que podamos empezar a registrar nuestros avances. 🐘💙`
    );
  ...
  const milestoneDetails = [
    { key: 'JULY_15', label: tr('Milestone 1: July 15', 'Hito 1: 15 de julio'), dateLabel: tr('July 15 Check-in', 'Reporte del 15 de julio') },
    { key: 'JULY_30', label: tr('Milestone 2: July 30', 'Hito 2: 30 de julio'), dateLabel: tr('July 30 Check-in', 'Reporte del 30 de julio') },
    { key: 'AUGUST_15', label: tr('Milestone 3: August 15', 'Hito 3: 15 de agosto'), dateLabel: tr('August 15 Check-in', 'Reporte del 15 de agosto') },
    { key: 'AUGUST_30', label: tr('Milestone 4: August 30', 'Hito 4: 30 de agosto'), dateLabel: tr('August 30 Check-in', 'Reporte del 30 de agosto') }
  ];
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-slate-500 font-semibold">{tr('Loading Summer Buddy Up Dashboard...', 'Cargando el Panel de Summer Buddy Up...')}</p>
      </div>
    );
  }
    ...
    <div className={`space-y-6 font-sans relative ${(showWaiverModal || showAddMemberModal) ? 'blur-sm select-none pointer-events-none' : ''}`}>
      ...
      {team.status === 'PENDING_ADMIN_APPROVAL' && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-4 shadow-sm mb-6">
          ...
          <div>
            <h3 className="font-bold text-amber-800 text-sm">{tr('Awaiting Administrator Approval', 'Esperando Aprobación del Administrador')}</h3>
            <p className="text-amber-700 text-xs mt-1 leading-relaxed">
              {tr('Your cohort registration is complete! An administrator is currently reviewing your team details. You can continue to invite sub-coaches and add students while you wait.', '¡El registro de tu grupo está completo! Un administrador está revisando los detalles de tu equipo. Mientras esperas, puedes seguir invitando sub-entrenadores y agregando estudiantes.')}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[9999] p-4 select-text pointer-events-auto">
          <div className="bg-white max-w-xl w-full rounded-2xl shadow-2xl border border-slate-100 overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-gradient-to-r from-sky-500 to-indigo-600 px-6 py-5 text-white flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-xl font-bold">{tr('Edit Team', 'Editar Equipo')}</h3>
                <p className="text-xs text-indigo-100 mt-1">{tr('Add a new student or sub-coach to the roster', 'Agrega un nuevo estudiante o sub-entrenador a la lista')}</p>
              </div>
            ...
            <div className="p-6 overflow-y-auto grow">
              <div className="flex space-x-2 mb-6">
                ...
                >
                  {tr('Add Sub-Coach', 'Agregar Sub-Entrenador')}
                </button>
              </div>

              <form id="add-member-form" onSubmit={handleAddMember} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Full Name', 'Nombre Completo')}</label>
                  <input
                    type="text"
                    required
                ...
                {addMemberType === 'coach' && (
                  <>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Email Address', 'Correo Electrónico')}</label>
                      <input
                        type="email"
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Phone Number', 'Número de Teléfono')}</label>
                      <input
                        type="tel"
                        required
                ...
                {addMemberType === 'student' && (
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Grade Level', 'Grado Escolar')}</label>
                        <select required value={addMemberForm.grade} onChange={(e) => setAddMemberForm({...addMemberForm, grade: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                          <option value="">{tr('Select Grade', 'Selecciona el Grado')}</option>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('School District', 'Distrito Escolar')}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Role', 'Rol')}</label>
                        <select required value={addMemberForm.classification} onChange={(e) => setAddMemberForm({...addMemberForm, classification: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                          <option value="Inclusion Buddy">{tr('Inclusion Buddy (IEP)', 'Amigo de Inclusión (IEP)')}</option>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Awards', 'Premios')}</label>
                        <select required value={addMemberForm.award_delivery_type} onChange={(e) => setAddMemberForm({...addMemberForm, award_delivery_type: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                          <option value="IN_PERSON_ONLY">{tr('In-Person Only', 'Solo en Persona')}</option>
                          <option value="VIRTUAL_DIGITAL">{tr('Virtual/Digital', 'Virtual/Digital')}</option>
                        </select>
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">{tr('Parent Email', 'Correo del Padre/Madre')}</label>
            ...
            <div className="border-t border-slate-100 p-6 bg-slate-50 shrink-0">
              ...
              >
                {addingMember ? tr('Saving...', 'Guardando...') : addMemberType === 'student' ? tr('Add Student', 'Agregar Estudiante') : tr('Add Sub-Coach', 'Agregar Sub-Entrenador')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[9999] p-4 select-text pointer-events-auto">
          <div className="bg-white max-w-xl w-full rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
            <div className="bg-gradient-to-r from-indigo-600 to-sky-500 px-6 py-5 text-white">
              <h3 className="text-xl font-bold">{tr('Parent Partnership Consent & Waiver', 'Consentimiento y Exención de Sociedad de Padres')}</h3>
              <p className="text-xs text-indigo-100 mt-1">{tr('Review legal releases to join team', 'Revisa las exenciones legales para unirte al equipo')} "{team.team_name}"</p>
            </div>
            ...
            <form onSubmit={handleWaiverSubmit} className="p-6 space-y-4">
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 text-xs leading-relaxed text-slate-600 border-b border-slate-100 pb-4">
                <p>
                  {tr('Welcome! You have been added as a Partnering Parent/Sub-Coach for the Summer Buddy Up program. To safeguard student privacy and establish safe guidelines, please review and accept the agreements below:', '¡Bienvenido! Has sido agregado como Padre/Madre Socio/Sub-Entrenador del programa Summer Buddy Up. Para proteger la privacidad de los estudiantes y establecer pautas de seguridad, por favor revisa y acepta los siguientes acuerdos:')}
                </p>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-150 space-y-2">
                  <h5 className="font-bold text-slate-800">{tr('1. Offline Consent Signatures', '1. Firmas de Consentimiento en Papel')}</h5>
                  <p>
                    {tr('All students on the roster must have offline signatures captured from their respective legal guardians before participating. By checking the boxes, you agree to coordinate and verify that physical safety release sheets are kept on file.', 'Todos los estudiantes en la lista deben tener firmas en papel de sus respectivos tutores legales antes de participar. Al marcar las casillas, aceptas coordinar y verificar que las hojas físicas de exención de seguridad se mantengan archivadas.')}
                  </p>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-150 space-y-2">
                  <h5 className="font-bold text-slate-800">{tr('2. No-Face Photography Compliance', '2. Cumplimiento de Fotografía Sin Rostro')}</h5>
                  <p>
                    {tr("Aaria's Blue Elephant promotes child safety and protects children from online facial recognition trackers. When filming check-in videos, please avoid direct eye-level frontal face shots of students. Film from behind, focus on hands/actions, use creative camera angles, or obtain explicit parent permission first.", "Aaria's Blue Elephant promueve la seguridad infantil y protege a los niños de rastreadores de reconocimiento facial en línea. Al filmar los videos de reportes, evita tomas frontales directas del rostro de los estudiantes. Filma desde atrás, enfócate en manos/acciones, usa ángulos de cámara creativos, u obtén primero el permiso explícito de los padres.")}
                  </p>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-150 space-y-2">
                  <h5 className="font-bold text-slate-800">{tr('3. Liability Release & PII Safety', '3. Exención de Responsabilidad y Seguridad de Datos Personales')}</h5>
                  <p>
                    {tr("Aaria's Blue Elephant is a 501(c)(3) (Entity No. B20250299015). Activities are self-guided and voluntary. You release Aaria's Blue Elephant from direct liabilities during independent cohort project sessions. Do not include students' full names or locations in YouTube video titles.", "Aaria's Blue Elephant es una entidad 501(c)(3) (Entidad No. B20250299015). Las actividades son autoguiadas y voluntarias. Liberas a Aaria's Blue Elephant de responsabilidades directas durante las sesiones de proyectos independientes del grupo. No incluyas los nombres completos ni ubicaciones de los estudiantes en los títulos de los videos de YouTube.")}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  ...
                  <span className="text-xs text-slate-700">
                    {tr('I accept the general liability waiver release policies.', 'Acepto las políticas generales de exención de responsabilidad.')}
                  </span>
                </label>
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  <input
                  ...
                  />
                  <span className="text-xs text-slate-700">
                    {tr('I agree to follow the "No-Face" visual filming guardrails in all milestone uploads.', 'Acepto seguir las pautas de filmación "Sin Rostro" en todas las subidas de hitos.')}
                  </span>
                </label>
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  <input
                  ...
                  />
                  <span className="text-xs text-slate-700">
                    {tr('I certify I will protect child PII and submit only unlisted YouTube links.', 'Certifico que protegeré los datos personales de los niños y solo enviaré enlaces de YouTube no listados.')}
                  </span>
                </label>
              </div>
              ...
              >
                {submittingWaiver ? tr('Signing...', 'Firmando...') : tr('Sign Waiver & Open Dashboard', 'Firmar Exención y Abrir el Panel')}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <span className="bg-white/20 backdrop-blur-md text-white text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full">
              {tr('Summer Buddy Cohort', 'Grupo Summer Buddy')}
            </span>
            <h2 className="text-3xl font-extrabold mt-1.5">{team.team_name}</h2>
            <p className="text-sm text-sky-50 mt-1">{tr('Focus Area:', 'Área de Enfoque:')} <strong className="text-white">{team.focus_area}</strong></p>
          </div>

          <div className="flex flex-col items-start md:items-end gap-1.5">
            <div className="flex flex-col md:flex-row md:items-center gap-2">
              <span className="text-xs font-semibold text-sky-100">{tr('Status:', 'Estado:')}</span>
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                ...
                {team.status === 'PENDING_CONSENT' ? tr('⏳ PENDING PARENT CONSENT', '⏳ PENDIENTE DE CONSENTIMIENTO DE PADRES') : team.status === 'PENDING_ADMIN_APPROVAL' ? tr('⏳ PENDING ADMIN APPROVAL', '⏳ PENDIENTE DE APROBACIÓN DEL ADMINISTRADOR') : tr('✓ ACTIVE COHORT', '✓ GRUPO ACTIVO')}
              </span>
            ...
            <div className="text-[11px] font-medium text-sky-100">
              {isPrizeEligible ? (
                <span className="bg-emerald-600/30 border border-emerald-500/30 text-emerald-200 px-2 py-0.5 rounded-full font-bold">
                  🎉 {tr(`Final Award Ceremony Eligible (${submittedCount}/4)`, `Elegible para la Ceremonia Final de Premios (${submittedCount}/4)`)}
                </span>
              ) : (
                <span className="bg-white/10 px-2 py-0.5 rounded-full">
                  {tr(`Submit any 2 milestones to qualify for awards (${submittedCount}/4)`, `Envía cualquier 2 hitos para calificar para los premios (${submittedCount}/4)`)}
                </span>
              )}
            </div>
          </div>
      ...
      {!isTeamActive && (
        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-2xl text-xs md:text-sm text-amber-800">
          <div className="font-bold mb-1">⚠️ {tr('Checklist Locked (Pending Co-Parent Consent)', 'Lista Bloqueada (Pendiente de Consentimiento de Copadres/madres)')}</div>
          {tr('Our safety policy requires all listed co-parents to log in and sign their safety waiver. Once all sub-coaches accept, the timeline checklist will automatically unlock!', 'Nuestra política de seguridad requiere que todos los copadres/madres registrados inicien sesión y firmen su exención de seguridad. ¡Una vez que todos los sub-entrenadores acepten, la lista de la línea de tiempo se desbloqueará automáticamente!')}
        </div>
      )}
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800 border-b border-slate-100 pb-2 flex justify-between items-center text-sm md:text-base">
              <span>{tr('Coaches & Parents', 'Entrenadores y Padres')}</span>
              <span className="text-[10px] text-slate-400 font-normal">{tr('Pull Login Matched', 'Coincidencia de Inicio de Sesión')}</span>
            </h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-xs p-2.5 rounded-xl bg-slate-50 border border-slate-150">
                <div>
                  <div className="font-bold text-slate-800"> {currentUser.user_metadata?.full_name || currentUser.email?.split('@')[0] || tr('Primary Coordinator', 'Coordinador Principal')} ({tr('You', 'Tú')})</div>
                  <div className="text-[10px] text-sky-600 font-semibold mt-0.5">{currentUser.email}</div>
                </div>
                <div className="text-right">
                  <span className="text-[10px] bg-emerald-500 text-white font-bold px-2 py-0.5 rounded-full">{tr('Owner', 'Propietario')}</span>
                </div>
              </div>
                  ...
                  <div key={index} className="p-2.5 rounded-xl border border-slate-150 space-y-1.5 text-xs bg-white">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">{tr('Co-Parent', 'Copadre/madre')}</div>
                      </div>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                        ...
                        {isConsented ? tr('Consented', 'Consintió') : tr('Pending', 'Pendiente')}
                      </span>
                    </div>

                    <div className="text-[10.5px] text-slate-500 space-y-0.5 border-t border-slate-50 pt-1.5 font-mono">
                      <div>
                        {tr('Email:', 'Correo:')} <span className="text-slate-700">{isConsented || isHeadCoach ? coach.email : maskEmail(coach.email)}</span>
                      </div>
                      <div>
                        {tr('Phone:', 'Teléfono:')} <span className="text-slate-700">{isConsented || isHeadCoach ? coach.phone : maskPhone(coach.phone)}</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            {isHeadCoach && (
              <div className="border border-indigo-100 bg-indigo-50/50 p-3.5 rounded-xl space-y-2.5">
                <div className="text-xs font-bold text-indigo-900">{tr('Invite Co-Parents / Partners', 'Invitar Copadres/madres / Socios')}</div>
                <p className="text-[10px] text-slate-500 leading-normal">
                  {tr('Since we protect user privacy and use zero transaction emails, copy and share the link/text directly with co-parents to sign waivers!', '¡Como protegemos la privacidad de los usuarios y no usamos correos automáticos, copia y comparte el enlace/texto directamente con los copadres/madres para que firmen las exenciones!')}
                </p>
                <button
                  type="button"
                ...
                >
                  {copySuccess ? tr('✓ Copied Invite to Clipboard!', '✓ ¡Invitación Copiada al Portapapeles!') : tr('Copy WhatsApp / Text Invite', 'Copiar Invitación de WhatsApp / Texto')}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800 border-b border-slate-100 pb-2 text-sm md:text-base">
              {tr(`Roster Students (${students.length})`, `Estudiantes de la Lista (${students.length})`)}
            </h3>
                  ...
                  <div className="pl-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                        ...
                        {student.classification === 'Inclusion Buddy' ? tr('Buddy', 'Amigo') : tr('Mentor', 'Mentor')}
                      </span>
                    </div>

                    <div className="grid grid-cols-2 gap-y-1.5 mt-2 text-[10px] text-slate-500 font-semibold border-t border-slate-200/50 pt-2">
                      <div>{tr('Grade:', 'Grado:')} <span className="text-slate-850">{student.grade}</span></div>
                      <div>{tr('District:', 'Distrito:')} <span className="text-slate-850">{student.school_district}</span></div>
                      <div className="col-span-2">
                        {tr('Delivery:', 'Entrega:')} <span className="text-indigo-600">{student.award_delivery_type === 'IN_PERSON_ONLY' ? tr('🏆 Local In-Person Ceremony', '🏆 Ceremonia Local en Persona') : tr('✉️ Virtual Digital', '✉️ Digital Virtual')}</span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 p-5 md:p-6 rounded-2xl shadow-sm space-y-6">
            <div className="border-b border-slate-100 pb-3 flex justify-between items-center">
              <div>
                <h3 className="font-bold text-slate-800 text-base md:text-lg">{tr('Buddy Up Check-In Milestones', 'Hitos de Avance de Buddy Up')}</h3>
                <p className="text-xs text-slate-500 mt-0.5">{tr('Flexible dates: Submit any 2 out of 4 check-ins for award certification.', 'Fechas flexibles: envía cualquier 2 de 4 reportes de avance para la certificación de premios.')}</p>
              </div>
            </div>
            ...
            {!buddyUpConfig?.checkins_enabled && (
              <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl text-amber-800 text-sm font-semibold">
                {tr('Check-ins are currently disabled by the administrator. Please check back later.', 'Los reportes de avance están deshabilitados actualmente por el administrador. Por favor vuelve más tarde.')}
              </div>
            )}
                  ...
                  <div key={milestone.key} className={`border rounded-2xl p-4 transition-all ${
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-400 font-medium">{tr('Cohort Milestones Log', 'Registro de Hitos del Grupo')}</span>
                        </div>
                      </div>
                    ...
                    {checkIn && (
                      <div className="mt-4 border-t border-slate-100 pt-3.5 space-y-2.5 text-xs text-slate-750">
                        <div className="flex items-center gap-1.5 mb-3">
                          <span className="font-bold text-slate-500">{tr('Video Link:', 'Enlace del Video:')}</span>
                          <a
                          ...
                          >
                            🎬 {tr('View YouTube Video', 'Ver Video de YouTube')}
                          </a>
                        </div>
                        ...
                        <div className="text-[10px] text-slate-400 font-semibold text-right mt-2">
                          {tr('Submitted at', 'Enviado el')} {new Date(checkIn.submitted_at).toLocaleDateString()}
                        </div>
                      </div>
                    )}
                    ...
                    {isFormActive && (
                      <form onSubmit={(e) => handleCheckInSubmit(e, milestone.key as any)} className="mt-4 border-t border-slate-100 pt-4 space-y-4 text-xs">
                        ...
                        <div className="space-y-3">
                          <div>
                            <label className="block font-bold text-slate-700 mb-1">{tr('Unlisted YouTube URL', 'Enlace de YouTube No Listado')} <span className="text-red-500">*</span></label>
                            <input
                              type="url"
                              required
                              value={youtubeUrl}
                              onChange={(e) => setYoutubeUrl(e.target.value)}
                              placeholder={tr('e.g. https://www.youtube.com/watch?v=...', 'ej. https://www.youtube.com/watch?v=...')}
                            ...
                            <p className="text-[9px] text-slate-400 mt-1">
                              ⚠️ {tr('To protect child privacy, configure your YouTube video privacy settings as', 'Para proteger la privacidad de los niños, configura la privacidad de tu video de YouTube como')} <strong>{tr('Unlisted', 'No listado')}</strong>. {tr("Do not use student full names in titles!", '¡No uses los nombres completos de los estudiantes en los títulos!')}
                            </p>
                          </div>
                          ...
                          {buddyUpConfig?.checkin_questions.map((question, i) => (
                            <div key={i}>
                              ...
                              <textarea
                                ...
                                placeholder={tr('Type your answer here...', 'Escribe tu respuesta aquí...')}
                                rows={3}
                        ...
                        >
                          {submittingCheckIn ? tr('Uploading...', 'Subiendo...') : tr('Submit Log Entries', 'Enviar Entradas del Registro')}
                        </button>
                      </form>
                    )}
                  </div>
