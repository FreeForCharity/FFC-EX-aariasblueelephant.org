...
const Events: React.FC = () => {
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-[80vh] flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
            <h1 className="text-3xl font-bold text-white mb-2">Playgroups & Classes</h1>
            <p className="text-slate-400">Join our inclusive sessions designed for all abilities.</p>
        </div>
        <div className="mt-4 md:mt-0 flex space-x-2 bg-slate-800 p-1 rounded-lg">
            {(['upcoming', 'all', 'past'] as Tab[]).map((tab) => (
                <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-all capitalize ${
                        activeTab === tab 
                        ? 'bg-brand-cyan text-slate-900 shadow-lg' 
                        : 'text-slate-400 hover:text-white hover:text-slate-300 hover:bg-slate-700'
                    }`}
                >
                    {tab} Events
                </button>
      ...
      {filteredEvents.length === 0 ? (
        <div className="py-20 px-4 text-center text-slate-400 bg-slate-800/30 rounded-2xl border border-slate-700/50 border-dashed">
        ...
        >
          ...
          <Link to={`/events/${activeEvent.id}`} className="block group cursor-pointer">
            <div className="bg-brand-card border border-slate-700 rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 ease-in-out hover:border-brand-cyan/50 hover:shadow-brand-cyan/10">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="relative h-64 lg:h-auto overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 hidden lg:flex items-center justify-center">
                     <span className="flex items-center bg-brand-cyan/90 text-slate-900 font-bold px-6 py-2 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                ...
                {/* Content Section */}
                <div className="p-8 flex flex-col justify-between h-full bg-brand-card relative z-30">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                        <h2 className="text-3xl font-bold text-white mb-4 leading-tight group-hover:text-brand-cyan transition-colors">{activeEvent.title}</h2>
                    </div>
                    ...
                    <div className="space-y-4 mb-8">
                        <div className="flex items-center text-slate-300">
                            <div className="w-8 flex justify-center mr-3"><Calendar className="h-5 w-5 text-brand-cyan" /></div>
                            <span className="font-medium">{new Date(activeEvent.date).toLocaleDateString()}</span>
                        </div>
                        <div className="flex items-center text-slate-300">
                            <div className="w-8 flex justify-center mr-3"><Clock className="h-5 w-5 text-brand-cyan" /></div>
                            <span className="font-medium">{activeEvent.time}</span>
                        </div>
                        <div className="flex items-center text-slate-300">
                            <div className="w-8 flex justify-center mr-3"><MapPin className="h-5 w-5 text-brand-cyan" /></div>
                            <span className="font-medium">{activeEvent.location}</span>
                        </div>
                        <div className="flex items-center text-slate-300">
                            <div className="w-8 flex justify-center mr-3"><Users className="h-5 w-5 text-brand-cyan" /></div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4 pt-6 border-t border-slate-800">
                     ...
                     <div className="flex w-full sm:w-auto gap-2 justify-between sm:justify-start relative z-30">
                        <button 
                            ...
                            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-slate-700 hover:bg-slate-800 transition-colors"
                        >
                            ...
                            <span className="text-slate-300 font-medium text-sm">{likeCounts[activeEvent.id] || 0}</span>
                        </button>
                        
                        <button 
                            onClick={(e) => handleShare(e, activeEvent.id)}
                            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-slate-700 hover:bg-slate-800 transition-colors group min-w-[100px]"
                        >
          ...
          <button 
              onClick={prevSlide}
              className="absolute top-1/2 -left-4 lg:-left-12 transform -translate-y-1/2 bg-slate-800/80 hover:bg-brand-cyan text-white p-3 rounded-full backdrop-blur-sm border border-slate-700 transition-all shadow-xl z-10"
          ...
          <button 
              onClick={nextSlide}
              className="absolute top-1/2 -right-4 lg:-right-12 transform -translate-y-1/2 bg-slate-800/80 hover:bg-brand-cyan text-white p-3 rounded-full backdrop-blur-sm border border-slate-700 transition-all shadow-xl z-10"
              ...
              {filteredEvents.map((_, idx) => (
                  <button
                      ...
                      className={`h-2 rounded-full transition-all duration-300 ${
                          idx === safeActiveIndex ? 'w-8 bg-brand-cyan' : 'w-2 bg-slate-700 hover:bg-slate-600'
                      }`}
      ...
      {/* Host CTA */}
      <div className="mt-16 bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 text-center border border-slate-700 shadow-lg">
        <h3 className="text-2xl font-bold text-white mb-4">Host a Playgroup?</h3>
