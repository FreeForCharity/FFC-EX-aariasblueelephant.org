  ...
  if (!event) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mb-6 text-slate-400">The event you are looking for does not exist or has been removed.</p>
        <Link to="/events">
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="relative z-20 mx-auto -mt-20 max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-2xl bg-brand-card border border-slate-700 shadow-2xl">

          {/* Header Section */}
          <div className="border-b border-slate-700 p-6 sm:p-10">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h1 className="text-3xl font-bold text-white sm:text-4xl lg:text-5xl">{event.title}</h1>
              </div>

              <div className="flex gap-3">
                <button
                  onClick={() => setIsLiked(!isLiked)}
                  className="rounded-full border border-slate-600 bg-slate-800 p-3 text-slate-400 transition-all hover:border-brand-pink hover:text-brand-pink"
                >
                  <Heart className={`h-6 w-6 ${isLiked ? 'fill-brand-pink text-brand-pink' : ''}`} />
                </button>
                <button
                  onClick={handleShare}
                  className="rounded-full border border-slate-600 bg-slate-800 p-3 text-slate-400 transition-all hover:border-brand-cyan hover:text-brand-cyan"
                >
                  {isCopied ? <Check className="h-6 w-6 text-green-400" /> : <Share2 className="h-6 w-6" />}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3">
            {/* Left Column: Details */}
            <div className="col-span-2 p-6 sm:p-10 border-b lg:border-b-0 lg:border-r border-slate-700">
              <h2 className="text-xl font-bold text-white mb-4">About this Event</h2>
              <p className="text-lg leading-relaxed text-slate-300 mb-8 whitespace-pre-line">
                {event.description}
              </p>
            ...
            <div className="bg-slate-800/30 p-6 sm:p-10">
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-400">Availability</p>
                ...
                <div className="pt-6 mt-6 border-t border-slate-700">
                  {event.registered < event.capacity ? (
                    <Button fullWidth size="lg" className="shadow-xl shadow-brand-cyan/10">
                      Register Now
                    </Button>
                  ) : (
